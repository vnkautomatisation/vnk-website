...
// GET /:id/signing-url — obtenir le lien de signature HelloSign pour le client
router.get('/:id/signing-url', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(
            'SELECT hellosign_request_id, status FROM contracts WHERE id = $1 AND client_id = $2',
            [req.params.id, req.user.id]
        ...
        const contract = result.rows[0];
        if (!contract.hellosign_request_id)
            return res.json({ success: false, message: 'Aucune demande de signature active pour ce contrat.' });

        // Récupérer le lien de signature via HelloSign
        const { getSignatureStatus } = require('../services/hellosign');
        const statusResult = await getSignatureStatus(contract.hellosign_request_id);

        if (statusResult.success && statusResult.signatures && statusResult.signatures.length > 0) {
            const signingUrl = statusResult.signatures[0].sign_url || statusResult.signingUrl;
            if (signingUrl) {
                return res.json({ success: true, signingUrl });
            }
        }

        res.json({ success: false, message: 'Lien de signature indisponible. Contactez VNK.' });
    } catch (e) {

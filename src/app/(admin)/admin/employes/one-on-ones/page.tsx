  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2"><MessageSquare className="h-5 w-5 text-[#0F2D52]" />Réunions 1-on-1</h1>
          <p className="text-sm text-muted-foreground">Mes 1-on-1 (comme employé ou manager).</p>
        </div>
        <Link href="/admin/employes/one-on-ones/new"><Button>Planifier</Button></Link>
      </div>

      <section>
        <h2 className="text-sm font-semibold mb-2 uppercase tracking-wider">À venir ({upcoming.length})</h2>
        {upcoming.length === 0 ? (
          <Card className="p-6 text-center text-sm text-muted-foreground">Aucune réunion planifiée.</Card>
        ) : (
      ...
      {past.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold mb-2 uppercase tracking-wider">Passées</h2>
          <div className="space-y-2">
            {past.map((m) => (
              <Link key={m.id} href={`/admin/employes/one-on-ones/${m.id}`}>

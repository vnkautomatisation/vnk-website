...
export function EmailTemplateDialog({
  ...
  useEffect(() => {
    ...
    if (template) {
      ...
      setSubject("");
      setBodyHtml(
        `<p>${t("salutation_client")}</p>
<p></p>
<p>${t("formule_politesse")}<br />${t("equipe_vnk")}</p>`
      );
  ...
  const handleSave = () => {
    startTransition(async () => {
      ...
      if (r.success) {
        toast.success(t("modele_enregistre"));
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="p-0 gap-0 max-w-4xl max-h-[92vh] overflow-hidden flex flex-col">
        <div className="bg-[#0F2D52] text-white px-6 py-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            <DialogTitle className="text-white text-base">
              {defaultLabel ?? (mode === "create" ? t("nouveau_modele_email") : template?.key)}
            </DialogTitle>
        ...
        <div className="border-b px-6">
          <div className="flex gap-1">
            ...
            <button onClick={() => setTab("preview")} className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px ${tab === "preview" ? "border-[#0F2D52] text-[#0F2D52]" : "border-transparent text-muted-foreground"}`}>
              <Eye className="h-3.5 w-3.5 inline mr-1" />{t("email_template_dialog_apercu")}</button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {tab === "edit" && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <Label className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">{t("cle_interne")}</Label>
              ...
              <div>
                <Label className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">{t("objet_champ")}</Label>
                <Input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder={t("devis_quote_number_pret")} className="mt-1" />
              </div>
              <div>
                <Label className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">{t("corps_html")}</Label>
                <div className="mt-1">
          ...
          {tab === "text" && (
            <>
              <p className="text-xs text-muted-foreground">
                {t("version_texte_courriel_clients_ayant")}
              </p>
              <Textarea
                value={bodyText}
                onChange={(e) => setBodyText(e.target.value)}
                rows={18}
                placeholder={t("bonjour_client_name_10_10")}
          ...
          {tab === "preview" && (
            <div className="space-y-2">
              <div className="rounded-md border border-blue-200 bg-blue-50 p-2.5 text-xs text-blue-900">
                ...
                {t("apercu_donnees_fictives_jean_tremblay")}
              </div>
              <iframe
          ...
          {tab === "vars" && (
            <>
              <p className="text-xs text-muted-foreground">{t("email_template_dialog_cliquez_sur_une_variable_pour_l_inserer")}<code className="bg-muted px-1.5 py-0.5 rounded">{`{{nom_variable}}`}</code>
              </p>

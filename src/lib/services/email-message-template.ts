...
<html lang="${locale}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>VNK Automatisation</title></head>
  ...
  <div style="max-width:600px;margin:0 auto;background:#ffffff">
    <div style="background:linear-gradient(135deg,#0F2D52 0%,#15406d 50%,#0F2D52 100%);padding:24px;color:#ffffff">
      <table style="width:100%"><tr>
        ...
        <p style="margin:4px 0 0;font-size:13px;color:rgba(255,255,255,0.7)">${t("chat_bonjour", { name: opts.clientName ?? t("chat_cher_client") })}</p></td>
      </tr></table>
    </div>
    <div style="padding:24px;font-size:15px;line-height:1.6">
      ...
      <div style="margin-top:24px;padding-top:16px;border-top:1px solid #e2e8f0;text-align:center">
        <a href="${opts.portalUrl}" style="display:inline-block;background:#0F2D52;color:#ffffff;text-decoration:none;padding:10px 20px;border-radius:6px;font-size:14px;font-weight:600">${t("chat_ouvrir_le_portail")}</a>
      </div>
    </div>
    <div style="background:#f8fafc;padding:16px 24px;border-top:1px solid #e2e8f0;font-size:11px;color:#64748b;text-align:center">
      ...
      <p style="margin:4px 0 0">${t("chat_pour_repondre")}</p>
    </div>
  </div>
  ${pixel}
</body></html>`;
